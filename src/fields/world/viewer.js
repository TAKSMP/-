...
//  - ダッシュ：うごいている あいだ（カーソルキー/WASD、または 画面を ドラッグする
//    ジョイスティックの どちらでも）、それを うごかしている 指とは べつに もう1本 指で
//    がめんの どこかを おさえ続けると DASH_MULT ばい の はやさに なる（固定ボタンでは なく
//    指の くみあわせ。スマホは 2本指＝ジョイスティック＋もう1本、PCは キー＋指1本）。
//    Shiftキーだけでも どうよう（指が ない ばあいの だいたい）。
//    touches：いま おしている 指の かず（drag は そのうち ジョイスティックを うごかす
//    1本だけを おう）。cancel() で いっしょに クリアする。
//  - tiles.request は まいフレーム よばず、見ている 区画（いち＋ズーム）が かわった ときだけ よぶ
//    （557この はいれつを まいフレーム なめると おもい）。ズームだけ かわって いちが おなじ ときも
//    ひろく なった ぶんの 区画を とりに いく ひつようが あるので、キーに zoom も いれて ある。
...
export async function mountWorld(host,{map,gameUrl,referenceUrl,tileManifest,tileBaseUrl,resolveTile,loadTile=null,artBg=null,artScale=1,walkZoom=8,startZoom,detailZoom=6,signal,onPosition=()=>{},drawPlayer=null,startWalking=false}){
 ...
 // maxCache：いちばん ひろい ズーム(0.7)で 画面いっぱいに ひつような 区画すうより おおきく
 // ないと、はしっこが えいきゅうに しろぬけに なる（request()が candidates を これで きるため）。
 const tiles=new TileStore(tileManifest,{maxCache:60,resolve:resolveTile||(file=>new URL(file,tileBaseUrl).href),load:loadTile||undefined});
 ...
 let width=1,height=1,dpr=1,zoom=1,cx=map.width/2,cy=map.height/2,mode='overview',source=false,collision=false,route=[],routeIndex=0,last=0,raf=0,alive=true,paused=false,select=false,lastTileKey='';const tileSize=tileManifest?.tileSize||64;const DASH_MULT=2;const player={...map.spawn};let travel=0,moving=false,facing=map.spawn.facing||'down',lastX=player.x,lastY=player.y;let vx=0,vy=0,pointer=null,drag=null;const keys=new Set();const touches=new Set();
 ...
 function cancel(){vx=vy=0;keys.clear();pointer=null;drag=null;touches.clear();knob.style.transform='translate(0px,0px)';homeStick();}
 ...
 canvas.addEventListener('pointerdown',e=>{canvas.focus({preventScroll:true});touches.add(e.pointerId);if(!drag)drag={id:e.pointerId,x:e.clientX,y:e.clientY,cx,cy,moved:false};canvas.setPointerCapture(e.pointerId);},opts);
 ...
 canvas.addEventListener('pointerup',e=>{touches.delete(e.pointerId);if(!drag||drag.id!==e.pointerId)return;const moved=drag.moved;drag=null;if(moved){if(mode==='walk')cancel();return;}const point=screenWorld(e.clientX,e.clientY),target=nearestRoad(map,mask,point.x,point.y,Math.min(20,Math.max(2,12/zoom)));if(!target){message('この位置には読み取れた道路がありません。');return;}if(select){player.x=target.x;player.y=target.y;walk();return;}if(mode==='walk'){route=findRoute(map,mask,player,target)||[];routeIndex=0;message(route.length?'選んだ道路へ移動します。':'この道路への接続は元画像から確認できていません。');}else{cx=target.x;cy=target.y;zoom=4;}},opts);
 canvas.addEventListener('pointercancel',()=>{touches.clear();drag=null;if(mode==='walk')cancel();},opts);
 ...
 host.addEventListener('keydown',e=>{const k=e.key.toLowerCase();if(['arrowup','arrowdown','arrowleft','arrowright','w','a','s','d'].includes(k)){e.preventDefault();keys.add(k);route=[];}else if(k==='shift'){keys.add(k);}},opts);host.addEventListener('keyup',e=>keys.delete(e.key.toLowerCase()),opts);window.addEventListener('blur',cancel,opts);host.addEventListener('focusout',e=>{if(!host.contains(e.relatedTarget))cancel();},opts);document.addEventListener('visibilitychange',()=>{cancel();last=0;},opts);
 function frame(t){if(!alive)return;const dt=last?Math.min(.04,(t-last)/1000):0;last=t;
 moving=false;if(mode==='walk'&&!paused&&!document.hidden){const moveKeyHeld=keys.has('arrowright')||keys.has('d')||keys.has('arrowleft')||keys.has('a')||keys.has('arrowdown')||keys.has('s')||keys.has('arrowup')||keys.has('w');const joystickActive=!!(drag&&drag.moved);const freeTouches=touches.size-(joystickActive?1:0);const speedMul=(keys.has('shift')||((moveKeyHeld||joystickActive)&&freeTouches>=1))?DASH_MULT:1;let dx=vx+(keys.has('arrowright')||keys.has('d')?1:0)-(keys.has('arrowleft')||keys.has('a')?1:0),dy=vy+(keys.has('arrowdown')||keys.has('s')?1:0)-(keys.has('arrowup')||keys.has('w')?1:0);const n=Math.max(1,Math.hypot(dx,dy));move(map,mask,player,dx/n*map.speed*speedMul*dt,dy/n*map.speed*speedMul*dt);
 let distance=map.speed*speedMul*dt;while(routeIndex<route.length&&distance>0){const p=route[routeIndex],len=Math.hypot(p.x-player.x,p.y-player.y);if(len<.001){routeIndex++;continue;}const amount=Math.min(distance,len),oldX=player.x,oldY=player.y;move(map,mask,player,(p.x-player.x)/len*amount,(p.y-player.y)/len*amount);distance-=amount;if(Math.hypot(player.x-oldX,player.y-oldY)<amount*.5){route=[];break;}if(Math.hypot(p.x-player.x,p.y-player.y)<.01)routeIndex++;}
 ...
 ctx.setTransform(dpr,0,0,dpr,0,0);ctx.fillStyle='#e9e5d8';ctx.fillRect(0,0,width,height);if(artBg&&!source&&mode!=='walk'){artBg.draw(ctx,{cx:cx*artScale,cy:cy*artScale,zoom:zoom/artScale,width,height,dpr});}else{ctx.save();ctx.translate(width/2,height/2);ctx.scale(zoom,zoom);ctx.translate(-cx,-cy);ctx.imageSmoothingEnabled=true;const detail=!source&&zoom>=detailZoom,view={left:cx-width/2/zoom,top:cy-height/2/zoom,right:cx+width/2/zoom,bottom:cy+height/2/zoom};if(detail){const tk=`${Math.floor(cx/tileSize)}_${Math.floor(cy/tileSize)}_${zoom.toFixed(2)}`;if(tk!==lastTileKey){lastTileKey=tk;tiles.request(view);}}if(!detail||!tiles.getStatus().ready)ctx.drawImage(source&&reference?reference:game,0,0,map.width,map.height);if(detail)tiles.draw(ctx,view);ctx.restore();}if(collision){ctx.save();ctx.translate(width/2,height/2);ctx.scale(zoom,zoom);ctx.translate(-cx,-cy);ctx.drawImage(getOverlay(),0,0);ctx.restore();}
 ...
 return{getPosition:()=>({...player}),getTileStatus:()=>tiles.getStatus(),getMode:()=>mode,setOverview(value){if(value)showOverview();else walk();},setZoom(value){zoom=Math.max(.15,Math.min(walkZoom,value));},setPaused(value){paused=!!value;cancel();route=[];},destroy(){alive=false;cancelAnimationFrame(raf);tiles.destroy();ro.disconnect();events.abort();host.innerHTML='';host.classList.remove('world-view');}};
}
