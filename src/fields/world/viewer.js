...
export async function mountWorld(host,{map,gameUrl,referenceUrl,tileManifest,tileBaseUrl,resolveTile,loadTile=null,artBg=null,artScale=1,walkZoom=8,signal,onPosition=()=>{},drawPlayer=null,startWalking=false}){
 ...
 const tiles=new TileStore(tileManifest,{resolve:resolveTile||(file=>new URL(file,tileBaseUrl).href),load:loadTile||undefined});
 ...
 ctx.setTransform(dpr,0,0,dpr,0,0);ctx.fillStyle='#e9e5d8';ctx.fillRect(0,0,width,height);if(artBg&&!source&&mode!=='walk'){artBg.draw(ctx,{cx:cx*artScale,cy:cy*artScale,zoom:zoom/artScale,width,height,dpr});}else{ctx.save();ctx.translate(width/2,height/2);ctx.scale(zoom,zoom);ctx.translate(-cx,-cy);ctx.imageSmoothingEnabled=true;const detail=!source&&zoom>=6,view={left:cx-width/2/zoom,top:cy-height/2/zoom,right:cx+width/2/zoom,bottom:cy+height/2/zoom};if(detail){const tk=`${Math.floor(cx/tileSize)}_${Math.floor(cy/tileSize)}`;if(tk!==lastTileKey){lastTileKey=tk;tiles.request(view);}}if(!detail||!tiles.getStatus().ready)ctx.drawImage(source&&reference?reference:game,0,0,map.width,map.height);if(detail)tiles.draw(ctx,view);ctx.restore();}if(collision){ctx.save();ctx.translate(width/2,height/2);ctx.scale(zoom,zoom);ctx.translate(-cx,-cy);ctx.drawImage(getOverlay(),0,0);ctx.restore();}
