...
export async function mountWorld(host,{map,gameUrl,referenceUrl,tileManifest,tileBaseUrl,resolveTile,loadTile=null,artBg=null,artScale=1,walkZoom=8,startZoom,detailZoom=6,signal,onPosition=()=>{},drawPlayer=null,startWalking=false}){
 ...
 let width=1,height=1,dpr=1,zoom=1,cx=map.width/2,cy=map.height/2,mode='overview',source=false,collision=false,route=[],routeIndex=0,last=0,raf=0,alive=true,paused=false,select=false,lastTileKey='';const tileSize=tileManifest?.tileSize||64;const DASH_MULT=2;let buttonDash=false;const player={...map.spawn};let travel=0,moving=false,facing=map.spawn.facing||'down',lastX=player.x,lastY=player.y;let vx=0,vy=0,pointer=null,drag=null;const keys=new Set();
 ...
 moving=false;if(mode==='walk'&&!paused&&!document.hidden){const speedMul=(buttonDash||keys.has('shift'))?DASH_MULT:1;let dx=vx+(keys.has('arrowright')||keys.has('d')?1:0)-(keys.has('arrowleft')||keys.has('a')?1:0),dy=vy+(keys.has('arrowdown')||keys.has('s')?1:0)-(keys.has('arrowup')||keys.has('w')?1:0);const n=Math.max(1,Math.hypot(dx,dy));move(map,mask,player,dx/n*map.speed*speedMul*dt,dy/n*map.speed*speedMul*dt);
 ...
 return{getPosition:()=>({...player}),getTileStatus:()=>tiles.getStatus(),getMode:()=>mode,setOverview(value){if(value)showOverview();else walk();},setZoom(value){zoom=Math.max(.15,Math.min(walkZoom,value));},setDash(value){buttonDash=!!value;},setPaused(value){paused=!!value;cancel();route=[];},destroy(){alive=false;cancelAnimationFrame(raf);tiles.destroy();ro.disconnect();events.abort();host.innerHTML='';host.classList.remove('world-view');}};
}
